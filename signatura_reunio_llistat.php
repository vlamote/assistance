<html>
	...
	<body>
		<table border="0" width="100%" id="table2">
			<tr>
				<td width="10%"><p align="left"><font face="Verdana" size="1"><a href="/assistencia/signatura_reunio_formulari.php">Nova consulta</a></td>
...
///////////////////////////////////
				...
				<table id='estadistiquesprofes' width='200px' class='tablesorter'>
					...
					/*********************************************************/
					$sql01 = "SELECT * FROM mdl_log WHERE ((course='377') AND (module LIKE '%attforblock%') AND (ip LIKE '192.168.%') AND (action LIKE '%attendance taked%') AND (time>='$dataentrada1') AND (time<='$dataentrada2')) order by time asc";
					$result01=mysql_query($sql01, $conexion);
					while($row01=mysql_fetch_row($result01)){

						$datasessio=date("Y-m-d (l) H:i",$row01[1]);
						$profe=$row01[9];
						$curs=$row01[4];
						$ip_origen=$row01[3];
					...
					<tr>
						...
						<td align='left' bgcolor='$color' width='100px'><font face='Verdana' size='1'>$profe</a></font></td>
					</tr>";
...
?>
<form action="signatura_reunio_llistat_pdf.php" method="POST">
	<input type="hidden" name="signatura_reunio_llistat_pdf" value='<?php echo serialize($registrespdf) ?>'</input>
